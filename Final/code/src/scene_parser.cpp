...
void SceneParser::parsePerspectiveCamera() {
    ...
    if (isDF){
        ...
        getToken(token);
        assert (!strcmp(token, "focusLen"));
        focusLen = readdouble();
    }
    getToken(token);
    assert (!strcmp(token, "}"));
    camera = new PerspectiveCamera(center, direction, up, width, height, angle_radians, isDF, lenRadius, lenSampleNum, focusLen);
}
...
void SceneParser::parsePhoton() {
    ...
    assert (!strcmp(token, "{"));
    getToken(token);
    assert (!strcmp(token, "emitPhoton"));
    emitPhoton = readInt();
    getToken(token);
    assert (!strcmp(token, "maxInMap"));
    maxInMap = readInt();
    getToken(token);
    assert (!strcmp(token, "samplePhotons"));
    sample_photons = readInt();
    getToken(token);
    assert (!strcmp(token, "sampleDist"));
    sample_dist = readInt();
    getToken(token);
    assert (!strcmp(token, "}"));
}
...
void SceneParser::parseLights() {
    ...
    while (num_lights > count) {
        ...
        }else if (strcmp(token, "DirectionalRecLight") == 0) {
            lights[count] = parseDirectionalRecLight();
        }else if (token[0]=='#') {
            continue;
        } else {
...
Light *SceneParser::parseDirectionalRecLight() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    getToken(token);
    assert (!strcmp(token, "{"));
    getToken(token);
    ...
    getToken(token);
    assert (!strcmp(token, "}"));
    return new DirectionalRecLight(position, d, color, up, dx, dy);
}
...
Material *SceneParser::parseMaterial() {
    ...
    double diffidx = 1, specidx = 0, shininess = 0;
    double reflidx = 0;
    ...
    while (true) {
        ...
        if (strcmp(token, "mColor") == 0) {
            ...
            diffidx = readdouble();
        }else if (strcmp(token, "specidx") == 0) {
            specidx = readdouble();
    ...
    textDir.normalize();
    auto *answer = new Material(mColor, textDir, absorb, diffidx, specidx, shininess, reflidx, refridx, refrN, filename, textcoff);
    return answer;
}
...
Object3D *SceneParser::parseObject(char token[MAX_PARSER_TOKEN_LENGTH]) {
    ...
    }else if (!strcmp(token, "FinitePlane")) {
        answer = (Object3D *) parseFinitePlane();
...
FinitePlane *SceneParser::parseFinitePlane() {
    Vector3f vertex[4];
    char token[MAX_PARSER_TOKEN_LENGTH];
    getToken(token);
    assert (!strcmp(token, "{"));
    ...
    double offset = readdouble();
    getToken(token);
    assert (!strcmp(token, "v0"));
    vertex[0] = readVector3f();
    getToken(token);
    assert (!strcmp(token, "v1"));
    vertex[1] = readVector3f();
    getToken(token);
    assert (!strcmp(token, "v2"));
    vertex[2] = readVector3f();
    getToken(token);
    assert (!strcmp(token, "v3"));
    vertex[3] = readVector3f();
    getToken(token);
    assert (!strcmp(token, "}"));
    assert (current_material != nullptr);
    return new FinitePlane(normal, offset, current_material, vertex);
}
